...
class ViewLog(Frame):
    ...
    def _calc_prev_sunday(self, date):
        return date - dt.timedelta(days=int(date.strftime("%w")))

    def _calc_next_saturday(self, date):
        SATURDAY = 6
        return date + dt.timedelta(SATURDAY - int(date.strftime("%w")))
    



    def _calc_month_start(self, date):
        return dt.datetime(year=date.year, month=date.month, day=1)


    def _calc_month_end(self, date):
        days_in_month = calendar.monthrange(date.year, date.month)
        return dt.datetime(year=date.year, month=date.month, day=days_in_month[1])
    ...
    def reset_calendars(self):
        ...
        if mode == DAY:
            self.draw_calendars(today, today)
        elif mode == WEEK:
            if storedsettings.LOG_MODE == CURRENT:
                self.draw_calendars(self._calc_prev_sunday(today), self._calc_next_saturday(today))
            else: # LOG_MODE == UPTILL
                self.draw_calendars(self._calc_week_ago(), today)
        elif mode == MONTH:
            if storedsettings.LOG_MODE == CURRENT:
                self.draw_calendars(self._calc_month_start(today), self._calc_month_end(today))
            else: # LOG_MODE == UPTILL
                self.draw_calendars(self._calc_month_ago(), today)
        elif mode == TASK:
